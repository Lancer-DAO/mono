...
const QuestDetails = () => {
  ...
  const bountyStateColor = (state: string) => {
    return {
      ...
      "text-neutral600 bg-[#EDC9FF] border-[#E2C2F2]": state === BountyState.IN_PROGRESS,
      "text-white bg-[#3D3D3D] border-[#333]": state === BountyState.COMPLETE,
      "text-neutral600 bg-[#FFBCB5] border-[#F2B0AA]": state === BountyState.AWAITING_REVIEW,
          ...
          {currentBounty?.escrow?.amount ? (
            <div className="flex items-center gap-[6px]">
              ...
              <Link
                ...
                target="true"
              >
                <ExternalLink className="text-neutral500" width={12} height={12} />
              </Link>
            </div>
          ) : (
            <FundCTA />
