...
export const insertIssue = async (params: IssueInsertParams) => {
  ...
  export const updateIssue = async (params: IssueUpdateParams) => {
    let query =
      `UPDATE issue SET`;
    query += `${!!params.state? ` state='${params.state}',` : '' }`
    query += `${!!params.timestamp? ` unix_timestamp='${params.timestamp}',` : '' }`
    query += `${!!params.escrowKey? ` escrow_key='${params.escrowKey}',` : '' }`
    query += `${!!params.hash? ` funding_hash='${params.hash}',` : '' }`
    query += `${!!params.amount? ` funding_amount=${params.amount},` : '' }`
    query += `${!!params.mint? ` funding_mint='${params.mint}',` : '' }`

    const fullQuery = query.slice(0, -1) + ` WHERE uuid='${params.uuid}'`

    console.log(fullQuery);
    const result = await DB.raw(fullQuery);
    return result;
  };
