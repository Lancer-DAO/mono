...
import { approveRequestPartialInstruction } from "@/escrow/sdk/instructions";
import { USDC_MINT } from "@/src/constants";
import { Escrow, LancerWallet } from "@/src/types";
import { decimalToNumber } from "@/src/utils";

export const approveRequestPartialFFA = async (
  submitter: PublicKey,
  acc: Escrow,
  wallet: LancerWallet,
  ...
  const partial_amount = decimalToNumber(acc.amount) / 2;
  debugger;
  let approveSubmitterIx = await approveRequestPartialInstruction(
    acc.timestamp,
    creator,
    submitter,
    tokenAddress,
