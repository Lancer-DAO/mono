...
import {
  ...
  approveRequestMultipleTransaction,
  approveRequestPartialInstruction,
  ...
  submitRequestMultipleInstruction,
...
describe("approve partial tests", () => {
  ...
  it("test createLancerTokenAccount works", async () => {
    ...
    const create_lancer_token_account_ix =
      await createLancerTokenAccountInstruction(WSOL_ADDRESS, program);
    ...
    assert.equal(
      lancer_token_account_info.mint.toString(),
  ...
  it("test partial payment works ", async () => {
    ...
    let tx = await provider.sendAndConfirm(
      new Transaction().add(create_FFA_ix),
      [creator]
    );
    ...
    tx = await provider.sendAndConfirm(
      new Transaction().add(addApproveSubmitterIx),
      [creator]
    );
    ...
    try {
      let approveRequestIx = await program.methods
        .approveRequest()
        .accounts({
    ...
    tx = await provider.sendAndConfirm(new Transaction().add(submitRequestIx), [
      submitter1,
    ]);
    console.log("submitRequest tx sig ", tx);

    // approve request(merge and send funds)(ApproveRequest)
    const submitter_token_account_before_balance =
      await provider.connection.getTokenAccountBalance(
        submitter1_wsol_account.address
      );
    const lancer_token_account_before_balance =
      await provider.connection.getTokenAccountBalance(
        lancer_dao_token_account
      );
    acc = await program.account.featureDataAccount.fetch(accounts[0].pubkey);
    ...
    try {
      await program.methods
        .approveRequestPartial(new anchor.BN(amount))
        .accounts({
          creator: creator.publicKey,
          submitter: submitter1.publicKey,
          payoutAccount: submitter1_wsol_account.address,
    ...
    assert.equal(
      acc.approvedSubmitters[0].toString(),
      submitter1.publicKey.toString()
    );
    assert.equal(
      acc.approvedSubmitters[1].toString(),
      PublicKey.default.toString()
    );
    assert.equal(
      acc.approvedSubmitters[2].toString(),
      PublicKey.default.toString()
    );
    assert.equal(
      acc.approvedSubmitters[3].toString(),
      PublicKey.default.toString()
    );
    assert.equal(
      acc.approvedSubmitters[4].toString(),
      PublicKey.default.toString()
    );
    ...
    tx = await provider.sendAndConfirm(
      new Transaction().add(approveRequestPartialIx),
      [creator]
    );
    console.log("approve Request Partial tx = ", tx);

    const submitter_token_account_after_balance =
      await provider.connection.getTokenAccountBalance(
        submitter1_wsol_account.address
      );
    const lancer_token_account_after_balance =
      await provider.connection.getTokenAccountBalance(
        lancer_dao_token_account
      );
  ...
  it("test partial payment works when admin creates bounty i.e no fees (single submitter)", async () => {
    ...
    tx = await provider.sendAndConfirm(
      new Transaction().add(fund_feature_ix),
      []
    );
    console.log("fundFeature transaction signature", tx);
